...
export default function Team() {
  ...
  const groups = groupTeamByRole(team || []).filter((g) => !g.separate);
  ...
  const leadershipCount = groups.reduce((n, g) => n + g.members.length, 0);
  ...
  return (
    <>
      <SeoHead
        ...
        description="Meet the Education Doorway directors, country managers and specialist teams."
        path="/team"
      />
      <PageHero
      ...
      <section className="bg-gradient-to-b from-slate-50 to-white pb-10 pt-10 sm:pt-12">
        <div className="container-x">
          <Reveal className="mb-10 flex flex-col items-start justify-between gap-4 sm:mb-12 sm:flex-row sm:items-end">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.14em] text-brand-600">Leadership</p>
              ...
              <p className="mt-2 max-w-xl text-sm leading-relaxed text-slate-500 sm:text-base">
                {leadershipCount
                  ? `${leadershipCount} team member${leadershipCount === 1 ? "" : "s"} in leadership.`
