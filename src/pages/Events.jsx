...
import { albumGalleryItems, splitEvents } from "../data/site";
...
export default function Events() {
  ...
  function setAlbumFilter(key) {
    const next = { tab: "gallery" };
    if (key) next.album = key;
    setSearchParams(next, { replace: true });
  }

  const { upcoming, past } = useMemo(() => splitEvents(events), [events]);
  const albums = useMemo(() => eventAlbums || [], [eventAlbums]);
  const gallery = useMemo(() => albumGalleryItems(albums), [albums]);
  const albumTabs = useMemo(() => [{ key: "", name: "All" }, ...albums], [albums]);

  const visibleGallery = useMemo(() => {
    if (!albumFromUrl) return gallery;
    return gallery.filter((item) => item.albumKey === albumFromUrl);
  }, [gallery, albumFromUrl]);
  ...
  const hero =
    active === "gallery"
      ? {
          title: "Event Gallery",
          subtitle: "Photos from Open Days, Assessment Days, Expos and more — organised by album.",
        }
      : active === "past"
        ? {
            title: "Past Events",
            ...
            <>
              ...
              {!albumFromUrl ? (
                <div className="space-y-10">
                  {albums.map((album) => {
                    const items = gallery.filter((g) => g.albumKey === album.key);
                    if (!items.length) return null;
                    return (
                      <div key={album.key}>
                        <h2 className="mb-4 font-display text-xl font-bold text-ink sm:text-2xl">{album.name}</h2>
                        <div className="columns-1 gap-4 sm:columns-2 lg:columns-3">
                          {items.map((item, i) => (
                            <Reveal key={item.key} delay={(i % 6) * 40} className="mb-4 break-inside-avoid">
                              <GalleryTile item={item} onOpen={setLightbox} />
                            </Reveal>
                          ))}
                        </div>
                      </div>
          ...
          >
            ...
            <div className="mt-3 flex flex-wrap items-center justify-between gap-3 px-1">
              <div>
                ...
                {lightbox.albumName ? (
                  <p className="text-sm text-slate-500">{lightbox.albumName}</p>
                ) : null}
              </div>
              <button
                type="button"
